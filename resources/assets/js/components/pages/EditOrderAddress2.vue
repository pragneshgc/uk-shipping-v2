<template>
    <div>
        ...
        <div class="modal">
                    ...
                    <section class="flexContent">
                        <div class="orderDetails">
                            ...
                            <ul>
                                ...
                                <li v-if="details.order.TrackingCode != '' && details.order.TrackingCode != null"><span>Tracking Code: </span>{{ details.order.TrackingCode }}</li>
            ...
            <transition name="fade">
                <form v-if="countries.length != 0 && companies.length != 0 && !loading && Object.keys(confirmationChanges).length == 0" @submit.prevent="save" class="pxp-form mb-20">
                    <div class="form-column">
                        <h3>Delivery Details</h3>
                        <div class="form-group form-group-column" v-for="(key, value) in details.order" :key="value">
                            <label :for="key">{{alias[value]}}</label>
                            <input v-if="!['CountryCode', 'DCountryCode', 'DeliveryID'].includes(value)" class="" :name="key" :placeholder="''" v-model="details.order[value]">
                            <select v-else-if="['DCountryCode', 'CountryCode'].includes(value)" v-model="details.order[value]">
                        ...
                        <tbody>
                            <tr v-for="(change, key) in confirmationChanges" :key="key">
                                ...
                                <td>
                                    ...
                                    <span v-else-if="key == 'DCountryCode'">{{ getCountryTitle(confirmationOld[key]) }}</span>
                                    <span v-else-if="key == 'CountryCode'">{{ getCountryTitle(confirmationOld[key]) }}</span>
                                    <span v-else>{{ confirmationOld[key] }}</span>
                                </td>
                                <td>
                                    <span v-if="key == 'DeliveryID'">{{ getCompanyTitle(confirmationChanges[key]) }}</span>
                                    <span v-else-if="key == 'DCountryCode'">{{ getCountryTitle(confirmationChanges[key]) }}</span>
                                    <span v-else-if="key == 'CountryCode'">{{ getCountryTitle(confirmationChanges[key]) }}</span>
            ...
            <transition name="fade">
                <div v-if="!loading" class="modal-footer">
                    <button v-if="!isEqual(details.order, details.oldOrder) || !isEqual(details.ups, details.oldUPS)" @click="save()" class="btn btnSize01 tertiaryBtn bigButton big-square-button">
                    ...
                    <button v-else-if="(!isEqual(details.order, details.oldOrder) || !isEqual(details.ups, details.oldUPS)) && saveConfirmation" @click="back()" class="btn btnSize01 tertiaryBtn bigButton big-square-button">
                        <span>
                            Back
                        </span> 
                    </button>
